...
- every server-owned shell now declares the shared GiveMeSpace transparent-helmet favicon family with ICO fallback, PNG browser and install icons, Apple touch icon, and app manifest so standard browser tabs, install surfaces, and Apple touch shortcuts use the same badge-free helmet silhouette
- the shared page titles are `GiveMeSpace`, `Admin Mode | GiveMeSpace`, `Login | GiveMeSpace`, `Enter Space | GiveMeSpace`, and `Shared Space | GiveMeSpace`
- every server-owned shell now also declares the same product-level Open Graph and Twitter card, using the title `GiveMeSpace | Browser-First AI Agent Runtime`, the description `Browser-first AI agent runtime for building your own AI spaces, tools, and workflows directly in the browser.`, and the local banner asset `server/pages/res/readme-banner.webp` served publicly at `https://givemespace.ai/pages/res/readme-banner.webp`; keeping that metadata on `/login` too ensures anonymous shares of `https://givemespace.ai/` still resolve to a proper preview after the auth redirect
