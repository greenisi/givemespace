...
- the renderer-side browser module must wait for Electron `did-attach` or `dom-ready` before calling guest-only webview methods, must set the guest `src` imperatively during registration instead of binding `<webview src>` reactively through Alpine state so initial loads are not aborted by ordinary toolbar state updates, must keep the `<x-browser>` shell and its immediate rendered child hit-transparent so the real browser surface receives direct input, must patch only the active webview host shadow-root frame chain so Electron's internal iframe fills the reserved shell while passive shadow-root siblings cannot intercept direct pointer input or reveal the modal's dark backing through transparent pages, and must reinstall the shared injected browser runtime after every full guest-page load instead of only the first placeholder page; browser-mode sessions still use the same toolbar and iframe fallback path, but without the desktop-only injected bridge features
