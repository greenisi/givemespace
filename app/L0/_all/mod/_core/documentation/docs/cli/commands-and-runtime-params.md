...
- `CLOUD_SHARE_URL`: the hosted-share receiver base URL used by the spaces share modal for uploads and by the receiver when generating returned share links; defaults to `share.agent-zero.ai` and is frontend-exposed
