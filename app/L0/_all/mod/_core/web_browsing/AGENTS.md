...
- `browser-webview.js`: renderer-side desktop `<webview>` helpers for partition naming, immediate navigation-state reads, vendored injection preludes, and runtime injection into desktop guest pages
...
- `browser-frame-inject.js`: inside-side bridge runtime fetched and evaluated by the packaged desktop host or addressed through iframe `postMessage`; it owns the built-in `ping`, `dom`, `content`, `navigation_state_get`, `location_navigate`, `history_back`, `history_forward`, and `location_reload` request handlers plus navigation-state event emission
- `vendor/dom-to-semantic-markdown.min.js`: vendored upstream browser bundle from `dom-to-semantic-markdown` used by the injected runtime's semantic-content handler
- `vendor/dom-to-semantic-markdown.LICENSE.txt`: upstream MIT license for the vendored semantic-markdown bundle
...
- the desktop `<webview>` path must expose the same `__spaceBrowserEmbedTransport__` bridge from its preload, must prepend the vendored semantic-markdown browser bundle before it injects the shared `/mod/_core/web_browsing/browser-frame-inject.js` runtime into each guest page after `dom-ready`, must treat that injection as document-scoped and reinstall it after every full guest-page navigation instead of only on the first placeholder page, and must keep toolbar navigation state fresh from both immediate embedder reads and bridge events
...
- the injected-side runtime must also register a `content` request handler that accepts the same selector payload shape as `dom`, runs the same DOM snapshot collection first, then converts each collected HTML string into semantic markdown through the vendored `dom-to-semantic-markdown` browser bundle, returning `{ document: "<semantic markdown>" }` for whole-document reads or an object keyed by selector for filtered reads
